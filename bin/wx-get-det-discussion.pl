...
# wx-get-discussion.pl - area forecast discussion from det/pontiac NWS
...
my $filename =  Config::get_value_for("htmldir") . Config::get_value_for("wx_det_discussion_output_file");
if ( $filename =~  m/^([a-zA-Z0-9\/\.\-_]+)$/ ) {
    $filename = $1;
} else {
    die "$dt : Bad filename $filename."; 
}
my $url = Config::get_value_for("det_forecast_discussion");
...
my $html_output = Web::display_page("Det/Pontiac - Area Forecast Discussion", "returnoutput");
